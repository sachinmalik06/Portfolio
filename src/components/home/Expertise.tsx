          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {certifications.map((cert: any, index: number) => (
              <motion.div
                ...
                className={`group relative overflow-hidden bg-card/40 backdrop-blur-md rounded-2xl border border-white/10 hover:border-primary/50 transition-all duration-300 ${cert.credential_url ? 'cursor-pointer hover:shadow-2xl hover:shadow-primary/20' : ''}`}
                ...
                layoutId={`cert-${cert.id || cert.title}`}
          ...
          {selectedCert && (
            <>
              ...
              <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 pointer-events-none">
                <motion.div
                  layoutId={`cert-${selectedCert.id || selectedCert.title}`}
